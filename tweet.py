...
def tweet_from_device(user_input):
  os.system('clear')
  auth = tweepy.OAuthHandler(os.getenv(f'DEVICE_{user_input}_API_KEY'), os.getenv(f'DEVICE_{user_input}_API_KEY_SECRET'))
  auth.set_access_token(os.getenv(f'DEVICE_{user_input}_ACCESS_TOKEN'), os.getenv(f'DEVICE_{user_input}_ACCESS_TOKEN_SECRET'))
  api = tweepy.API(auth)
